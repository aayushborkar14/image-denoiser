...
def gen_patches(file_name):
    ...
    for s in scales:
        ...
        img_scaled = cv2.resize(
            img, (w_scaled, h_scaled), interpolation=cv2.INTER_CUBIC
        )
...
def datagenerator(data_dir):
    ...
    for i, file_path in enumerate(file_list):
        ...
        for p in patches:
            if p.shape == (patch_size, patch_size, 3):
                data.append(p)
        if (i + 1) % 10 == 0:
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if (i + 1 + self.num_layers) % 2 == 0 and conv_feats_idx < len(conv_feats):
                x = x + conv_feats[-(conv_feats_idx + 1)]
                conv_feats_idx += 1
                x = self.relu(x)
...
if __name__ == "__main__":
    ...
    for epoch in trange(n_epoch):
        ...
        for noisy, clean in loader:
            ...
            psnr_loss += peak_signal_noise_ratio(clean_np, output_np, data_range=1.0)
            ssim += structural_similarity(
                clean_np.transpose(0, 2, 3, 1),
                output_np.transpose(0, 2, 3, 1),
                channel_axis=-1,
                data_range=1.0,
            )
        ...
        if (epoch + 1) % 10 == 0:
            torch.save(
                model.state_dict(),
                os.path.join(save_dir, f"rednet_epoch_{epoch + 1}.pth"),
            )

    # --- Inference on test set after training --- #
    model_path = os.path.join(save_dir, "rednet_epoch_100.pth")
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()

    test_files = glob.glob(os.path.join(test_data_dir, "*.jpg"))
    output_dir = "denoised_results_rednet"
